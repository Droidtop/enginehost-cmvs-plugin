...
int cmvs_menu_poll(cmvs_menus *m, int menu, cmvs_input *in, cmvs_scene *scene)
{
    ...
    if (cmvs_input_pressed(in, CMVS_FN_UP)) {
        cmvs_input_clear(in, CMVS_FN_UP);
        if (mm->current && mm->current->prev_id >= 0) hit = find(mm, mm->current->prev_id);
        else hit = last(mm);
    }
    if (cmvs_input_pressed(in, CMVS_FN_DOWN)) {
        cmvs_input_clear(in, CMVS_FN_DOWN);
    ...
    if (cmvs_input_pressed(in, CMVS_FN_CONFIRM)) mm->press_from = mm->current;
    if (cmvs_input_released(in, CMVS_FN_CONFIRM, NULL)) {
    ...
    if (!cmvs_input_held(in, CMVS_FN_CONFIRM)) {
        mm->press_from = NULL;
        if (mm->pressed) {
    ...
    if (cmvs_input_released(in, CMVS_FN_CANCEL, NULL)) {
        cmvs_input_clear(in, CMVS_FN_CANCEL);
