...
void cmvs_input_button(cmvs_input *in, int button, int down)
{
    ...
    if (button == 0) {
        ...
        if (down && !in->left_held) in->left_pressed = 1;
        if (!down && in->left_held) in->left_released = 1;
        in->left_held = down ? 1 : 0;
    } else if (button == 1) {
        if (down && !in->right_held) in->right_pressed = 1;
        if (!down && in->right_held) in->right_released = 1;
        in->right_held = down ? 1 : 0;
    }
}
...
{
    if (!in) return;
    if (direction < 0) in->up_pressed = 1;
    else if (direction > 0) in->down_pressed = 1;
}
